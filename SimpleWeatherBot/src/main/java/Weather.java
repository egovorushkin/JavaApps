...
public class Weather {
    ...
    public String currentWeather(Float lat, Float lon) throws IOException {
        ...
        if (myResponse.getInt("cod") == 200) {
            cityFound = myResponse.getString("name") + " (" +
                    myResponse.getJSONObject("sys").getString("country") + ")";
            ...
            responseToUser = String.format("Текущая погода в %s: %n Температура: %s %n Облачность: %s %n Погода: %s", cityFound, temp, cloudiness, weatherDesc);
        }
    ...
    public String forecastWeather(Float lat, Float lon) throws IOException {
        ...
        if (myResponse.getInt("cod") == 200) {
            ...
            responseToUser = String.format("Погода на ближайшие сутки в %s:%n %s%n", cityFound, convertWeatherToString(myResponse));
        }
    ...
    private String convertToString(JSONObject internalJSON) {
        ...
        responseToUser = String.format("Дата: %s%nПогода: %s%nМакс. темп.: %s%nМин. тем.: %s%n",
                date2, weather, tempMax, tempMin);
        return responseToUser;
    }
